  ...
  template: `
    <div class="input-group" style="flex-wrap: nowrap" [formGroup]="formGroup">
      <input
        ...
        (change)="change()"
      />
      <input type="hidden" [formControlName]="formItem.name" (ngModelChange)="setValue($event)" />
      <button #unitEl class="btn btn-outline-secondary" type="button" (click)="switchUnit()">
        {{ unit }}
      </button>
    </div>
    <ng-template #dropdown>
      <ul class="dropdown-menu" style="display: block;min-width: auto;position: static">
        <li *ngFor="let u of units" (click)="change(u)" [class.active]="u === unit">
...
export class UnitInputComponent {
  ...
  setValue(value: string) {
    if (value) {
      this.input = value.match(/([\d.-]|auto)/) ? value.match(/([\d.-]+|auto)/)![0] : '';
      this.unit = value.match(/(px|%|em|rem|vw|vh)$/) ? value.match(/(px|%|em|rem|vw|vh)$/)![0] : '\\';
    } else {
      this.input = '';
      this.unit = '\\';
    }
