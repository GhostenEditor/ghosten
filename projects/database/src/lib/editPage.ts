...
export function editPage(db: IDBDatabase, pageConfig: any): Promise<MessageEvent> {
  ...
  return resolveRequest<any>(
    ...
    if (request.result && request.result.id !== pageConfig.id) {
      ...
      return isLoopChain(pageConfig.id, pageConfig.parentId, configObjectStore).then(isLoop => {
        if (isLoop) {
          throw {
            type: 'error',
            name: 'parentId',
          ...
          return resolveRequest<any>(configObjectStore.get(pageConfig.id)).then(request => {
            if (!request.result) {
              throw {
                type: 'error',
                name: 'id',
...
function isLoopChain(targetId: any, parentId: any, objectStore: IDBObjectStore): Promise<boolean> {
  return resolveRequest<any>(objectStore.index('id').get(IDBKeyRange.only(parentId))).then(request => {
    if (request.result) {
      const _parentId = request.result.parentId === 'null' ? 'null' : +request.result.parentId;
