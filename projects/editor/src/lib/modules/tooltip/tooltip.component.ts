...
@Component({
  ...
  >
    <div
      ...
      [ngStyle]="{
        top: tooltipPosition === 'start' || tooltipPosition === 'end' ? '-.4rem' : '',
        left: tooltipPosition === 'bottom' || tooltipPosition === 'top' ? '-.4rem' : ''
      }"
    ></div>
...
export class TooltipComponent {
  ...
  show() {
    if (!this.elementRef.nativeElement.parentElement) {
      ...
      this._document.body.appendChild(this.elementRef.nativeElement);
    }
  ...
  public updatePosition(targetRect: DOMRect) {
    ...
    const maxContainerX = this._document.body.clientWidth - tooltipRect.width - 5;
    const minContainerX = 5;
    switch (this.tooltipPosition) {
      case 'top':
        arrowX = tooltipRect.width / 2;
        containerY = targetRect.top - tooltipRect.height;
        containerX = (targetRect.left + targetRect.right - tooltipRect.width) / 2;
      ...
      case 'start':
        ...
        containerY = (targetRect.top + targetRect.bottom - tooltipRect.height) / 2;
        break;
      case 'end':
      default:
        arrowY = tooltipRect.height / 2;
