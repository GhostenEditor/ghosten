...
@Component({
  ...
  template: `
    ...
    <div [@fade]="'in'" class="dropdown-menu show shadow" style="position:static;">
      <ng-container *ngFor="let menu of menus; let i = index">
        ...
        >
          ...
          <small class="text-desc text-muted ms-3" *ngIf="menu.desc">{{ menu.desc }}</small>
  ...
  mouseenter(event: MouseEvent, menu: Menu) {
    ...
    fromEvent(event.target as HTMLElement, 'mousemove')
      ...
      .subscribe(() => {
        ...
        this.menu.create(event as any, disabled ? [] : children ? children : [], this.level + 1);
      });
  }
...
export class ContextMenu {
  ...
  create(event: MouseEvent, menus: Menu[], level = 0) {
    ...
    this.overlayRefs.splice(level, this.overlayRefs.length).forEach((or: OverlayRef) => or.dispose());
