...
import { FormItem } from '../properties-form/types';
...
@Component({
  ...
  template: `
      ...
      <thead>
        <tr>
          <th
            class="text-truncate"
            style="width:3.8em"
            i18n="Table Column Header: Index"
          >
            序号
          </th>
          <th
            class="text-truncate"
            style="width:4em"
            i18n="Table Column Header: Category"
          >
            类型
          </th>
    ...
    <div class="d-grid gap-2 mt-2">
      <button class="btn btn-light" (click)="actionEdit()" i18n="Button: Add">
        添加
      </button>
    </div>
  `,
...
export class PanelBoardComponent {
  ...
  get actions(): IGtNode.Action[] {
    return Object.values(this.board.events!).reduce(
      (acc: any, value) => acc.concat(value),
      [],
    );
  }
